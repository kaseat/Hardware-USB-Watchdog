...
#define DEFAULT_0   ((uint8_t)0x1C)
#define DEFAULT_1    ((uint8_t)0x44)
#define DEFAULT_2    ((uint8_t)0x48)
#define DEFAULT_3    ((uint8_t)0x00)
#define INITIAL      ((uint32_t)0)

#ifdef __ICCSTM8__
#include "STM8S003F3.h"
__no_init uint8_t __eeprom dummy;
uint8_t __eeprom settings0 = DEFAULT_0;
uint8_t __eeprom settings1 = DEFAULT_1;
uint8_t __eeprom settings2 = DEFAULT_2;
uint8_t __eeprom settings3 = DEFAULT_3;
#endif
#ifdef __AVR__
#include <EEPROM.h>
#include <Arduino.h>
...
SettingsManager::SettingsManager()
{
	...
	while (f = pgm_read_byte(++p))
	{
		if (f != EEPROM[++e])
		{
			EEPROM[sizeof CompileTime + 1] = DEFAULT_0;
			EEPROM[sizeof CompileTime + 2] = DEFAULT_1;
			EEPROM[sizeof CompileTime + 3] = DEFAULT_2;
			EEPROM[sizeof CompileTime + 4] = DEFAULT_3;
