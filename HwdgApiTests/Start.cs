...
namespace HwdgApiTests
{
    ...
    public class Start
    {
        ...
        [TestMethod]
        public void CanSendStartWhenIdle()
        {
            Assert.AreEqual(Response.StartOk, hwdg.Start());
            Assert.IsTrue(hwdg.GetStatus().State.HasFlag(WatchdogState.IsRunning));
        }

        [TestMethod]
        public void CannotSendStartWhenMonitoring()
        {
            hwdg.Start();
            Assert.AreEqual(Response.Busy, hwdg.Start());
            Assert.IsTrue(hwdg.GetStatus().State.HasFlag(WatchdogState.IsRunning));
        }

        [TestMethod]
        public void CannotSendStartWhenWaitingForReboot()
        {
